...
With a schema, you can create a [fetch instance](/docs/zimic-fetch/api/2-fetch.md).

The `@zimic/fetch` package provides a [`createFetch`](/docs/zimic-fetch/api/1-create-fetch.md) function that takes the
schema as a type parameter and returns a thin wrapper around the native
...
Try our [examples](/docs/zimic/2-examples/2-examples.md) for more practical use cases of `@zimic/fetch` with popular
