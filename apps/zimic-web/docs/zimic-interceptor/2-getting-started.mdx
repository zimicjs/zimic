...
With the schema defined, you can now create an [HTTP interceptor](/docs/zimic-interceptor/api/2-http-interceptor.md).

`@zimic/interceptor` provides a [`createHttpInterceptor`](/docs/zimic-interceptor/api/1-create-http-interceptor.md)
function that takes the schema as a type parameter and returns an interceptor instance. It allows you to intercept HTTP
requests, validate their contents, and return mock responses, all automatically typed based on the schema.
