...
Note that [`@zimic/http`](/docs/zimic-http/1-index.md) and `@zimic/ws` are peer dependencies of `@zimic/interceptor`.
Install each or both depending on your needs. HTTP examples use `@zimic/http` schemas, while
[WebSocket examples](/docs/zimic-interceptor/guides/ws/1-local-websocket-interceptors.md) use `@zimic/ws` schemas. When
upgrading `@zimic/interceptor` to a new version, consider upgrading both schema packages as well to ensure that the
...
[WebSocket interceptor guide](/docs/zimic-interceptor/guides/ws/1-local-websocket-interceptors.md) while the dedicated
API pages are being added.
...
- Use the [WebSocket interceptor guide](/docs/zimic-interceptor/guides/ws/1-local-websocket-interceptors.md) when your
  tests need to mock typed WebSocket message flows with `@zimic/ws`.
