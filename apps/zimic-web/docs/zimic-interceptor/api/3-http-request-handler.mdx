...
1. **declaration**:
   ...
   If you need to bypass or reject the request instead of responding with a mock response, you can use the special
   `action` property:
   - **action** (optional): `'bypass' | 'reject'`

     If set to `'bypass'`, the request will be sent to the actual server instead of being mocked. On the other hand,
     setting it to `'reject'` will make the request fail with a network error. This behavior is similar to
     [unhandled request actions](/docs/zimic-interceptor/guides/http/9-unhandled-requests.md).
...
If the interceptor is local, the response declaration is immediately applied to the handler. If the interceptor is
remote, the declaration is asynchronously sent to the [interceptor server](/docs/zimic-interceptor/cli/1-server.md).
Because of the asynchronous nature of remote interceptors, make sure to await the handler before making requests.
