...
Declares a delay to apply before responding to matched intercepted requests.

The delay is applied after the request matches the handler but before the response is created. This is useful for
simulating network latency or testing timeout behavior.

<Tabs groupId="zimic-http-interceptor-type">
  <TabItem value="local" label="Local interceptor">

```ts
handler.delay(delayInMilliseconds);
handler.delay(minDelayInMilliseconds, maxDelayInMilliseconds);
handler.delay((request) => delayInMilliseconds);
```
...
```ts
await handler.delay(delayInMilliseconds);
await handler.delay(minDelayInMilliseconds, maxDelayInMilliseconds);
await handler.delay((request) => delayInMilliseconds);
```
...
1. **delayInMilliseconds**: `number | HttpRequestHandlerResponseDelayFactory<MethodSchema>`

   The delay to apply before responding, in milliseconds. It can be:
   - A **fixed number**: The same delay will be applied to all matched requests.
   - A **factory function**: A function that receives the intercepted request and returns the delay in milliseconds,
     either synchronously or asynchronously. This allows for dynamic delays based on the request.

2. **maxDelayInMilliseconds** (optional): `number`

   If provided along with the first argument (which must be a number in this case), the delay will be a random value
   between `delayInMilliseconds` (minimum) and `maxDelayInMilliseconds` (maximum), inclusive.

**Returns**: `HttpRequestHandler<Method, Path, StatusCode>`

The same handler, now including the delay configuration.
...
interceptor.get('/users').delay(100, 500).respond({ status: 200, body: [] });

// Computed delay based on request
interceptor
  .get('/users')
  .delay((request) => {
