...
Declares a delay to apply before returning the response for matched intercepted requests.

The delay can be a fixed number of milliseconds, a range of milliseconds (where a random value is selected), or a function that determines the delay based on the request.
...
handler.delay(200, 500);

// Function-based delay
handler.delay((request) => request.url.includes('slow=true') ? 1000 : 0);

// Async function-based delay
handler.delay(async (request) => {
...
// Random delay between 200ms and 500ms
await handler.delay(200, 500);

// Function-based delay
await handler.delay((request) => request.url.includes('slow=true') ? 1000 : 0);

// Async function-based delay
await handler.delay(async (request) => {
  await someAsyncOperation();
  return request.url.includes('slow=true') ? 1000 : 0;
});
```
...
1. **declaration**: `number | [number, number] | ((request: HttpInterceptorRequest<Path, MethodSchema>) => number | Promise<number>)`

   The delay declaration:
   - **number**: A fixed delay in milliseconds
   - **[number, number]**: A range of delays in milliseconds. A random value between the minimum (first number) and maximum (second number) will be selected for each request
   - **function**: A function that receives the intercepted request and returns the delay in milliseconds. Can be synchronous or asynchronous

**Returns**: `HttpRequestHandler<Method, Path, StatusCode>`

The same handler with the delay configuration applied.

**Examples**:

```ts
// Fixed delay for all requests
interceptor
  .get('/users')
  .delay(200)
...
// Conditional delay based on request
interceptor
  .get('/api/data')
  .delay((request) => request.url.includes('slow=true') ? 1000 : 0)
  .respond({ status: 200, body: { data: 'response' } });

// Delay with other handler methods
interceptor
  .get('/api/users')
