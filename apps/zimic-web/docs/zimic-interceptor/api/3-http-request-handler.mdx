...
Declares a delay to apply before returning a response for matched intercepted requests. The delay can be fixed, ranged,
or computed based on the request.
...
handler.delay({ min: minMilliseconds, max: maxMilliseconds });
...
await handler.delay({ min: minMilliseconds, max: maxMilliseconds });
...
1. **declaration**: `HttpRequestHandlerDelayDeclaration<Path, MethodSchema>`

   The delay declaration. It can be:

   - **Fixed delay** (`number`): A fixed delay in milliseconds to apply before returning each response.

     ```ts
     handler.delay(200); // Fixed 200ms delay
     ```

   - **Ranged delay** (`{ min: number; max: number }`): A random delay within the specified range (inclusive) in
     milliseconds. A random value will be selected within the interval `[min, max]` for each response.

     ```ts
     handler.delay({ min: 200, max: 500 }); // Random delay between 200ms and 500ms
     ```

   - **Computed delay** (`(request) => number | Promise<number>`): A function to dynamically compute the delay based on
     ...
     **Arguments**:
     ...
     1. **request**: `HttpInterceptorRequest<Path, MethodSchema>`
        ...
        The intercepted request to compute the delay for.

     **Returns**: `Promise<number> | number`

     The delay in milliseconds to apply before returning the response.

     ```ts
     handler.delay((request) => {
       return request.url.includes('slow=true') ? 1000 : 0;
     });
     ```

**Returns**: `HttpRequestHandler<Method, Path, StatusCode>`

The same handler, now including the specified delay.

If the interceptor is local, the delay is immediately applied to the handler. If the interceptor is remote, the delay is
asynchronously sent to the [interceptor server](/docs/zimic-interceptor/cli/1-server.md). Because of the asynchronous
nature of remote interceptors, make sure to await the handler before making requests.
...
const handler = interceptor
  .get('/users')
  .delay({ min: 200, max: 500 }) // Random delay between 200ms and 500ms
  .respond({ status: 200, body: [] });

// Computed delay
const handler = interceptor
  .get('/users')
  .delay((request) => (request.url.includes('slow=true') ? 1000 : 0))
  .respond({ status: 200, body: [] });
```
...
Clears any response declared with [`handler.respond()`](#handlerrespond), delays declared with
[`handler.delay()`](#handlerdelay), restrictions declared with [`handler.with()`](#handlerwith), and intercepted
requests, making the handler stop matching requests. The next handler, created before this one, that matches the same
method and path will be used if present. If not, the requests of the method and path will not be intercepted.
