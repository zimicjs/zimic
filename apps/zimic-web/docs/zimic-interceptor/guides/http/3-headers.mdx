...
Then, the request headers will be available in the `headers` property, such as in [`handler.with()`](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerwith), [computed responses](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerrespond), and [`handler.requests`](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerrequests). The headers are typed and validated according to your schema, providing type safety and autocompletion.
...
When sending a response to an intercepted request, you can set the headers in the [`handler.respond`](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerrespond) method with the `headers` property.
