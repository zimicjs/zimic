...
### Matching any value in a path param

Using a path param in an interceptor as declared in the schema, with a colon (`:`), allows you to match any value in
such parameter. This is useful when you want to create a generic handler supporting dynamic values.

By default, path params match a single segment. See
