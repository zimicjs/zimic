...
Then, the path params of the requests will be available in the `pathParams` property, such as in [`handler.with()`](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerwith), [computed responses](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerrespond), and [`handler.requests`](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerrequests).
...
By default, path params match a single segment, not including slashes (`/`). See [Declaring paths](/docs/zimic-http/guides/1-schemas.md#declaring-paths) for more details on how to declare repeating path params, which accept multiple segments.
...
Use [`interceptor.with()`](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerwith) in case you need validation or to limit a parameter to specific values.
