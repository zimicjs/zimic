...
To match any value in a path param, use it as declared in the schema. This is useful when you want to intercept requests
using dynamic values. By default, path params match a single segment. See
[Declaring paths](/docs/zimic-http/guides/1-schemas.md#declaring-paths) for more details on how to declare repeating
path params, which match multiple segments.
...
Use [`interceptor.with()`](/docs/zimic-interceptor/api/3-http-request-handler.mdx#handlerwith) in case you want to
