...
Restrictions can be declared using [`handler.with()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlerwith)
and specify conditions expected for the intercepted requests. These include
[headers](/docs/zimic-interceptor/guides/3-headers.mdx),
[search params](/docs/zimic-interceptor/guides/4-search-params.mdx), and
[bodies](/docs/zimic-interceptor/guides/6-bodies.mdx). A handler can have multiple restrictions, and all of them must be
met for the request to be matched and receive the mock response.
...
[`handler.with()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlerwith) method. The object can contain the
properties `headers`, `searchParams`, and `body`, which are used to check the intercepted requests.

#### Static header restrictions

To create static restrictions for headers, first define the request headers in your
[schema](/docs/zimic-interceptor/guides/3-headers.mdx#using-request-headers). Then, use the `header` property with the
headers that the request must contain.
...
[schema](/docs/zimic-interceptor/guides/4-search-params.mdx#using-request-search-params). Then, use the `searchParams`
...
[schema](/docs/zimic-interceptor/guides/6-bodies.mdx#using-request-bodies). For more details on using body types, such
as JSON, text, and form data, visit our [bodies guide](/docs/zimic-interceptor/guides/6-bodies.mdx).
...
[`handler.with()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlerwith) method. The function receives the
...
[`handler.with()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlerwith) method.
...
use [`handler.times()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlertimes), which takes a number or a
...
[unhandled requests guide](/docs/zimic-interceptor/guides/8-unhandled-requests.md).
...
[`handler.times()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlertimes) on its own only restricts how
many requests the handler should match. To really validate the number of requests, use
[`handler.checkTimes()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlerchecktimes) or
[`interceptor.checkTimes()`](/docs/zimic-interceptor/api/2-http-interceptor.md#interceptorchecktimes) at a point when
...
[`interceptor.checkTimes()`](/docs/zimic-interceptor/api/2-http-interceptor.md#interceptorchecktimes) in an `afterEach`
...
[`handler.checkTimes()`](/docs/zimic-interceptor/api/3-http-request-handler.md#handlerchecktimes) in each test.
