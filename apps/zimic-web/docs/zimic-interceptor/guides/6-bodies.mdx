...
For bodies to be correctly parsed by `@zimic/interceptor`, make sure that their `content-type` header is set
accordingly, such as `application/json` for JSON bodies, `multipart/form-data` for form data bodies,
`application/octet-stream`, `image/png`, `video/mp4`, and so on for binary bodies, and
`application/x-www-form-urlencoded` for URL-encoded bodies.
