...
describe('Type generation (OpenAPI)', () => {
  ...
  const helpOutput = [
    ...
    '  -f, --filter        One or more expressions to filter paths to generate types',
    '                      for.. Filters must follow the format `<method> <path>`, wh',
    '                      ere `<method>` is an HTTP method or `*`, and `<path>` is a',
    '                       literal path or a glob. For example, `GET /users`, `* /us',
    '                      ers`, `GET /users/*`, and `GET /users/**/*` are valid filt',
    '                      ers. If more than one filter is provided, they will be com',
    '                      bined with OR. Negative filters can be created by prefixin',
    '                      g the expression with `!`. For example, `!GET /users` will',
    '                       exclude paths matching `GET /users`. Paths in filters are',
    '                       case-sensitive.                     [array] [default: []]',
    '  -F, --filter-file   A path to a file containing expressions to filter paths to',
    '                       generate types for. One expression is expected per line a',
    '                      nd the format is the same as used in `--filter`. Comments',
    '                      are prefixed with `#`. Additional `--filter` expressions w',
    '                      ill be appended to the considered filters.        [string]',
  ].join('\n');
