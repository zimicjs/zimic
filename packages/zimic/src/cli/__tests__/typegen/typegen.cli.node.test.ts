...
describe('Type generation', () => {
  ...
  it('should show a help message', async () => {
    ...
    await usingIgnoredConsole(['log'], async (spies) => {
      ...
      expect(spies.log).toHaveBeenCalledWith(typegenHelpOutput);
    });
  });

  describe('OpenAPI', () => {
    const openAPIHelpOutput = [
      'zimic typegen openapi <input>',
      '',
      'Generate service types from an OpenAPI schema.',
      '',
      'Positionals:',
      '  input  The path to a local OpenAPI schema file or an URL to fetch it. YAML and',
      '          JSON are supported.                                [string] [required]',
      '',
      'Options:',
      '      --help             Show help                                     [boolean]',
      '      --version          Show version number                           [boolean]',
    ...
    it('should show a help message', async () => {
      ...
      await usingIgnoredConsole(['log'], async (spies) => {
        ...
        expect(spies.log).toHaveBeenCalledTimes(1);
        expect(spies.log).toHaveBeenCalledWith(openAPIHelpOutput);
      });
    });

    describe.each([
      { type: 'yaml', mode: 'local' },
      { type: 'yaml', mode: 'remote' },
      { type: 'json', mode: 'local' },
      { type: 'json', mode: 'remote' },
    ] as const)('Type $type, mode: $remote', (file) => {
      it('should generate types from a simple schema', async () => {
        const inputFilePath = path.join(fixtures.openapiDirectory, `simpleSchema.${file.type}`);
        const outputFilePath = path.join(fixtures.openapiDirectory, `simpleSchema.${file.type}.output.ts`);
