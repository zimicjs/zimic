...
export function declareUnhandledRequestHttpInterceptorTests(options: RuntimeSharedHttpInterceptorTestsOptions) {
  ...
  describe.each(HTTP_METHODS)('Method: %s', (method) => {
    ...
    describe.each([
      ...
      if (type === 'local') {
        it(`should show a warning when logging is enabled and an ${method} request is unhandled and bypassed`, async () => {
          await usingHttpInterceptor<{
            ...
            async (interceptor) => {
              ...
              await usingIgnoredConsole(['warn', 'error'], async (spies) => {
                const response = await fetch(joinURL(baseURL, '/users'), { method, headers: { 'x-value': '1' } });
      ...
      if (type === 'remote') {
        it(`should show an error when logging is enabled and an ${method} request is unhandled and rejected`, async () => {
          await usingHttpInterceptor<{
            '/users': {
              GET: UserMethodSchema;
              POST: UserMethodSchema;
