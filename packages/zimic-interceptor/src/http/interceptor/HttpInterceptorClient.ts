...
> {
  ...
  >(
    ...
    // Check if the declaration is an action response
    if (
      typeof responseDeclaration === 'string' &&
      (responseDeclaration === 'bypass' || responseDeclaration === 'reject')
    ) {
      // Return action for the worker to handle
      return { action: responseDeclaration };
    }

    // Handle as normal status-based response
