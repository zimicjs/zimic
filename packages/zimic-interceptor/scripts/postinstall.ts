...
async function patchMSWExports() {
  const mswPackageContentAsString = await fs.readFile(MSW_PACKAGE_PATH, 'utf-8');
  ...
  await fs.writeFile(MSW_PACKAGE_PATH, patchedMSWPackageContentAsString);
}
...
async function patchMSWBrowserEntry() {
  for (const indexFileName of ['index.js', 'index.mjs']) {
    ...
    const mswBrowserContent = await fs.readFile(mswBrowserPath, 'utf-8');
    ...
    await fs.writeFile(mswBrowserPath, patchedMSWBrowserContent);
  }
}
